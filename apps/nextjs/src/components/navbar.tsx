...
const ModeToggle = dynamic<unknown>(() => import("./theme-toggle").then((mod) => mod.default), {
            ...
            <div className="max-w-lg w-full lg:max-w-xs" ref={dropdownRef}>
              <div className="relative">
                ...
                {isOpen && citySuggestions.length > 0 && (
                  <div className="relative">
                    <SearchSuggestions suggestions={citySuggestions} selectedIndex={selectedIndex} onSelect={handleCitySelect} onMouseEnter={setSelectedIndex} />
                  </div>
                )}

                {error && (
                  <div className="absolute left-0 right-0 top-full mt-1 text-red-500 bg-background p-2 rounded-md shadow-lg" role="alert" aria-live="polite">
                    {error}
                  </div>
                )}

                {isSearching && (
                  <div className="absolute left-0 right-0 top-full mt-1 text-muted-foreground bg-background p-2 rounded-md shadow-lg" aria-live="polite">
                    Searching...
                  </div>
                )}
              </div>
