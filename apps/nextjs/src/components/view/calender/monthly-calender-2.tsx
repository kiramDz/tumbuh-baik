...
const getSuitabilitySummary = (data: any[]): SuitabilitySummary => {
  ...
  sortedData.forEach((item) => {
    ...
    const rain = getParameterValue(item.parameters, [
      "RR_imputed",
      "RR",
      "PRECTOTCORR",
    ]);
    const temp = getParameterValue(item.parameters, [
      "TAVG",
      "T2M",
      "T2M_MAX",
      "TMAX",
    ]);
    const humidity = getParameterValue(item.parameters, [
      "RH_AVG_preprocessed",
      "RH_AVG",
      "RH2M",
    ]);
...
const getPeriodCalendarGrid = (data: any[], startDate: Date, endDate: Date) => {
  ...
  days.forEach((date) => {
    const forecast = data.find(
      (item) =>
        format(new Date(item.forecast_date), "yyyy-MM-dd") ===
...
export default function PeriodCalendar() {
  ...
  const {
    ...
    queryFn: async () => {
      const fetchFunc =
        selectedModel === "lstm" ? getLSTMDaily : getHoltWinterDaily;
  ...
  const { periodRows, periodRanges } = useMemo(() => {
    ...
    Object.keys(dynamicPeriods).forEach((period) => {
      const { start, end } =
        dynamicPeriods[period as keyof typeof dynamicPeriods];
      newPeriodRows[period] = getPeriodCalendarGrid(forecastData, start, end);
      newPeriodRanges[period] =
        `${format(start, "d MMM yyyy", { locale: id })} - ${format(end, "d MMM yyyy", { locale: id })}`;
    });
                  ...
                  <TableRow key={day} className="hover:bg-muted/30">
                    ...
                    {periodRows[period][rowIdx]?.map((dayData, colIdx) => {
                      if (dayData && !dayData.isPlaceholder) {
                        const rain = getParameterValue(dayData.parameters, [
                          "RR_imputed",
                          "RR",
                          "PRECTOTCORR",
                        ]);
                        const temp = getParameterValue(dayData.parameters, [
                          "TAVG",
                          "T2M",
                          "T2M_MAX",
                          "TMAX",
                        ]);
                        const humidity = getParameterValue(dayData.parameters, [
                          "RH_AVG_preprocessed",
                          "RH_AVG",
                          "RH2M",
                        ]);
                        const radiation = getParameterValue(
                          dayData.parameters,
                          ["ALLSKY_SFC_SW_DWN"],
                        );
                              ...
                              >
                                <div className="flex flex-col items-center justify-center space-y-1">
                                  <div className="text-xs font-semibold">
                                    {format(
                                      new Date(dayData.forecast_date),
                                      "dd",
                                    )}
                                  </div>
                                  <div
                                ...
                                <div className="space-y-1.5 text-sm">
                                  ...
                                  >
                                    <div className="flex items-center gap-2">
                                      ...
                                      <span>
                                        Kelembaban:{" "}
                                        <strong>{humidity.toFixed(2)}%</strong>
                                      </span>
                                    </div>
                                    {suitability.criteria &&
                                      !suitability.criteria
                        ...
                        return (
                          <TableCell
                            key={colIdx}
                            className="text-center h-16 bg-muted/30"
                          >
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <CardDescription>
                Prediksi cuaca berdasarkan model{" "}
                {selectedModel === "lstm" ? "LSTM" : "Holt Winters"}
              </CardDescription>
            </div>
            <div className="flex items-center gap-2">
              <Badge
            ...
            <TabsList className="grid w-full grid-cols-3 h-auto">
              {["Periode-1", "Periode-2", "Periode-3"].map((period) => (
                <TabsTrigger
                  key={period}
                  value={period}
                  className="flex flex-col py-2 data-[state=active]:bg-teal-50 data-[state=active]:text-teal-700 data-[state=active]:border-teal-600"
                >
                  <span className="font-semibold">{period}</span>
                  <span className="text-xs text-muted-foreground">
                    {periodRanges[period as keyof typeof periodRanges]}
                  </span>
                </TabsTrigger>
              ))}
            </TabsList>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <div className="flex items-center gap-2 p-2.5 rounded-lg border bg-yellow-300/50">
              <div className="w-7 h-7 bg-yellow-400 border border-yellow-500 rounded-md flex items-center justify-center text-yellow-900 font-bold text-lg">
                ●
              </div>
              <div className="space-y-1">
