...
const WeatherChart = dynamic(
  () =>
    import("./weather-chart").then((mod) => ({ default: mod.WeatherChart })),
  {
    loading: () => (
...
const WeatherDashboard: React.FC<WeatherDashboardProps> = React.memo(
  ({ unit }) => {
    ...
    const selected = useMemo(() => {
      return (
        bmkgData?.find((item: any) => item.kode_gampong === selectedGampong) ??
        null
      );
    }, [bmkgData, selectedGampong]);
    ...
    const latestData = useMemo(
      () => getTodayWeather(selectedData),
      [selectedData],
    );
    const dailyForecast = useMemo(
      () => getDailyForecastData(bmkgData),
      [bmkgData],
    );
    const hourlyForecast = useMemo(
      () => getHourlyForecastData(selectedData),
      [selectedData],
    );
          ...
          <WeatherTabs defaultTab="weather">
            {isLoading ? (
              ...
              <div className="space-y-6">
                ...
                {latestData && selected && (
                  <WeatherMainDisplay latestData={latestData} unit={unit} />
                )}
