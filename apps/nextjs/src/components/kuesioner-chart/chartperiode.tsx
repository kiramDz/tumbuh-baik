...
export default function ChartPeriode() {
  const [selectedChart, setSelectedChart] =
    useState<ChartType>("periode-tanam");
  ...
  const processedData = useMemo(() => {
    ...
    const dataTanam = Object.entries(bulanTanamGroups).map(
      ([bulan, count]) => ({
        bulan,
        count,
        type: "tanam",
    ...
    const dataPanen = Object.entries(bulanPanenGroups).map(
      ([bulan, count]) => ({
        bulan,
        count,
        type: "panen",
    ...
    const ekonomiData = responses
      ...
      .map((item: any) => {
        ...
        return {
          ...
          percentage:
            responses.length > 0
              ? ((keuntungan / pendapatan) * 100).toFixed(1)
              : "0",
        };
      })
    ...
    const pengelolaan = Object.entries(pengelolaanGroups).map(
      ([status, count]) => ({
        status,
        count,
        percentage: ((count / responses.length) * 100).toFixed(1),
    ...
    const jenisLahan = Object.entries(jenisLahanGroups).map(
      ([jenis, count]) => ({
        jenis,
        count,
        percentage: ((count / responses.length) * 100).toFixed(1),
    ...
    responses.forEach((response: any) => {
      const kab =
        response.kab_kota || response.kabupaten || response.kabupaten_kota;
      if (kab && typeof kab === "string" && kab.trim() !== "") {
    ...
    // Statistics
    const validProduktivitas = responses.filter(
      (r: any) =>
        r.luas_panen_m2 > 0 &&
        r.gunca_kg > 0 &&
    ...
    const averageProduktivitas =
      validProduktivitas.length > 0
        ? validProduktivitas.reduce(
            (sum: number, r: any) =>
              sum + (r.gunca_kg / r.luas_panen_m2) * 10000,
            0,
          ) / validProduktivitas.length
        : 0;

    const averageKeuntungan =
      ekonomiData.length > 0
        ? ekonomiData.reduce(
            (sum: number, item: any) => sum + item.keuntungan,
            0,
          ) / ekonomiData.length
        : 0;

    const validLuasTanam = responses.filter(
      (r: any) => typeof r.luas_tanam_m2 === "number" && r.luas_tanam_m2 > 0,
    );
    const averageLuasTanam =
      validLuasTanam.length > 0
        ? validLuasTanam.reduce(
            (sum: number, r: any) => sum + r.luas_tanam_m2,
            0,
          ) / validLuasTanam.length
        : 0;

    const validFrekuensi = responses.filter(
      (r: any) =>
        typeof r.jml_tanam_padi_1th === "number" && r.jml_tanam_padi_1th > 0,
    );
    const averageFrekuensiTanam =
      validFrekuensi.length > 0
        ? validFrekuensi.reduce(
            (sum: number, r: any) => sum + r.jml_tanam_padi_1th,
            0,
          ) / validFrekuensi.length
        : 0;
  ...
  const renderChart = () => {
    ...
    switch (selectedChart) {
        ...
        return (
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-xs">
                Top 10 perbandingan pendapatan vs pengeluaran
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      case "pengelolaan":
        const totalPengelolaan = processedData.pengelolaan.reduce(
        ...
        return (
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-xs">
                Distribusi status pengelolaan lahan
              </CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  >
                    ...
                    <LabelList
                      ...
                      formatter={(value: number, entry: any) =>
                        `${entry.status}: ${((value / totalPengelolaan) * 100).toFixed(0)}%`
                      }
                    />
                  </Pie>
                </PieChart>
        ...
        if (!processedData.regional || processedData.regional.length === 0) {
          return (
            <Alert>
              ...
              <AlertDescription>
                Data kabupaten tidak tersedia. Field kab_kota tidak ditemukan
                dalam respons API.
              </AlertDescription>
            </Alert>
          );
  ...
  if (!processedData) {
    return (
      <Alert>
        <AlertCircle className="h-4 w-4" />
        <AlertDescription>Tidak ada data periode tersedia</AlertDescription>
      </Alert>
    );
  }
