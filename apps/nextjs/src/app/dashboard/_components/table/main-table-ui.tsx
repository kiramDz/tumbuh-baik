...
export function MainTableUI<TData, TValue>({ columns, data, pagination, sorting, export: exportProps, preprocessing }: DataTableProps<TData, TValue>) {
  ...
  const handlePreprocessingSuccess = (result: any) => {
    toast.success(`🎉 Preprocessing Completed! Successfully processed ${result.recordCount?.toLocaleString()} records. Quality: ${result.preprocessing_report?.quality_metrics?.completeness_percentage || 100}%`, {
      duration: 5000,
    ...
    <>
      <div className="space-y-4 ">
        <div className="w-full flex items-center gap-2 justify-end">
          ...
          {canPreprocess && (
            <Button variant="default" size="sm" onClick={handlePreprocessingClick} className="h-8 bg-green-600 hover:bg-green-700 text-white">
                ...
                <TableRow>
                  <TableCell colSpan={columns.length} className="h-48">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-sm text-muted-foreground mt-1">Data yang dicari tidak ditemukan</p>
                    </div>
                  </TableCell>
                </TableRow>
              )}
