...
const RecycleBinTable = () => {
  ...
  if (isLoading)
    return (
      <DataTableSkeleton
        columnCount={4}
        filterCount={2}
        cellWidths={["10rem", "30rem", "10rem", "10rem"]}
        shrinkZero
      />
    );
  ...
  const handleDelete = async (collectionName: string) => {
    try {
      await DeleteDatasetMeta(collectionName);
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem
              onClick={() => handleRestore(row.original.collectionName)}
            >
              Restore
            </DropdownMenuItem>
            <DropdownMenuItem
              onClick={() => handleDelete(row.original.collectionName)}
