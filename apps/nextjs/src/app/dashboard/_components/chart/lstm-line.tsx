...
function ParamChart({ param, data, mode }: ParamChartProps) {
  ...
  const avgValue =
    displayData.reduce((sum, d) => sum + d.value, 0) / displayData.length;
              ...
              content={
                <ChartTooltipContent
                  formatter={(value, name, item) => {
                    const label = item.payload.isHistorical
                      ? "Historis"
                      : "Prediksi";
...
export function LSTMLineChart() {
  const [viewMode, setViewMode] = useState<"forecast-only" | "combined">(
    "forecast-only",
  );
  ...
  const paramToCollection = useMemo(() => {
    ...
    const latestConfig =
      lstmConfigs.find((config: any) => config.status === "done") ||
      lstmConfigs[0];
  ...
  const { data: historicalDataMap, isLoading: isLoadingHistorical } = useQuery({
    ...
    enabled:
      viewMode === "combined" &&
      !!rawData &&
      Object.keys(paramToCollection).length > 0,
    refetchOnWindowFocus: false,
  });
