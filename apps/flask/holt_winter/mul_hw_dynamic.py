...
def post_process_forecast(forecast, param_name):
    ...
    if param_name == "RR" or param_name == "RR_imputed":  # Curah Hujan
        ...
        forecast = np.minimum(forecast, 500)  # Naikkan batas untuk hujan ekstrem
...
def grid_search_hw_params(train_data, param_name):
    ...
    # Transformasi data untuk multiplicative model (hindari 0)
    if param_name == "RR" or param_name == "RR_imputed":
        transformed_data = train_data + 0.01  # Tambahkan offset kecil
    else:
    ...
    split_point = int(len(transformed_data) * 0.8)
    train_split = transformed_data[:split_point]
    val_split = transformed_data[split_point:]
    
    # Pastikan indeks harian
    ...
    # Prioritaskan seasonal periods tahunan
    seasonal_periods_options = [365, 30, 7] if len(train_split) >= 730 else [30, 7] if len(train_split) >= 60 else [7]
    ...
    for seasonal_periods in seasonal_periods_options:
        for alpha, beta, gamma in itertools.product(alpha_range, beta_range, gamma_range):
            try:
                ...
                # Balikkan transformasi untuk forecast
                if param_name == "RR" or param_name == "RR_imputed":
                    forecast = np.maximum(forecast - 0.01, 0)  # Kurangi offset, pastikan non-negatif
                ...
                mse = mean_squared_error(val_split[:len(forecast)], forecast)
                rmse = np.sqrt(mse)
                mae = mean_absolute_error(val_split[:len(forecast)], forecast)
                mape = np.mean(np.abs((val_split[:len(forecast)] - forecast) / np.where(val_split[:len(forecast)] != 0, val_split[:len(forecast)], 1))) * 100
                ...
                if rmse < best_score:
                    ...
                    best_mse = mse
                    valid_models += 1
                    print(f"✓ New best: α={alpha}, β={beta}, γ={gamma}, seasonal_periods={seasonal_periods} | RMSE={rmse:.3f}, MAE={mae:.3f}, MAPE={mape:.3f}%, MSE={mse:.3f}")
                    
            except Exception as e:
                print(f"❌ Error for α={alpha}, β={beta}, γ={gamma}, seasonal_periods={seasonal_periods}: {str(e)}")
                continue
    
    # Fallback ke model non-seasonal
    if best_params is None:
        ...
        for alpha, beta in itertools.product(alpha_range, beta_range):
            try:
                ...
                # Balikkan transformasi untuk forecast
                if param_name == "RR" or param_name == "RR_imputed":
                    forecast = np.maximum(forecast - 0.01, 0)
...
def run_optimized_hw_analysis(collection_name, target_column, save_collection="holt-winter", config_id=None, append_column_id=True, client=None):
    ...
    try:
        ...
        # Transformasi data untuk multiplicative model
        if target_column == "RR" or target_column == "RR_imputed":
            param_data = param_data + 0.01
       ...
       # NEW: forecast hanya dari 2025-09-20 sampai 2026-09-19
