...
export const CalendarBody = ({ features, children }: CalendarBodyProps) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      <div key={day} className="relative flex h-full w-full flex-col gap-1 p-1 text-muted-foreground text-xs">
        {day}
  ...
  return (
    <div className="grid flex-grow grid-cols-7">
      {days.map((day, index) => (
        <div key={index} className={cn("relative aspect-square overflow-hidden border-t border-r", index % 7 === 6 && "border-r-0")}>
          {day}
        </div>
      ))}
    </div>
